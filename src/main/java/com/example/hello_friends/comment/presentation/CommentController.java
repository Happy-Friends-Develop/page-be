...
public class CommentController {
    ...
    public ResponseEntity<Resp<CommentResponse>> createComment(
        ...
        CommentResponse data = CommentResponse.from(comment);

        return ResponseEntity.status(HttpStatus.CREATED).body(Resp.ok(data));
    }

    // 특정 게시글의 댓글 목록 조회
    @Operation(summary = "게시글에 작성된 댓글을 조회할 수 있습니다.", description = "게시글에 매핑된 댓글을 조회합니다.")
    @GetMapping("/api/user/boards/{boardId}/comments")
    public ResponseEntity<Resp<List<CommentResponse>>> getComments(@PathVariable Long boardId) {
        List<CommentResponse> comments = commentService.getCommentsByBoardId(boardId);
        return ResponseEntity.ok(Resp.ok(comments));
    }

    // 댓글 수정
    @Operation(summary = "댓글을 수정합니다.", description = "댓글을 작성한 사람만 수정이 가능합니다.")
    @PatchMapping("/api/user/comments/{commentId}")
    public ResponseEntity<Resp<CommentResponse>> updateComment(
        ...
        return ResponseEntity.ok(Resp.ok(CommentResponse.from(updatedComment)));
    }
